...
const reloadIcon = document.querySelector('.fa'); //the 
...
function ascending(e) {
    ...
    allTasks.onsuccess = (e) => {
        ...
        for (let index = 0; index < Tasks.length; index++) {
            const li = document.createElement("li");
            li.setAttribute("data-task-id", Tasks[index].id);
            li.className = "collection-item";
            li.appendChild(document.createTextNode(Tasks[index].taskname));
            const link = document.createElement("a");
            link.className = "delete-item secondary-content";
            link.innerHTML = `
                     <i class="fa fa-remove"></i>
                    &nbsp;
                    <a href="./edit.html?id=${Tasks[index].id}"><i class="fa fa-edit"></i> </a>
                    `;
            li.appendChild(link);
            taskList.appendChild(li);
        }
