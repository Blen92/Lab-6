...
function ascending(e) {
    ...
    allTasks.onsuccess = (e) => {
        ...
        for (let index = 0; index < fTasks.length; index++) {
            const li = document.createElement("li");
            li.setAttribute("data-task-id", fTasks[index].id);
            li.className = "collection-item";
            li.appendChild(document.createTextNode(fTasks[index].taskname));
            const link = document.createElement("a");
            link.className = "delete-item secondary-content";
            link.innerHTML = `
                     <i class="fa fa-remove"></i>
                    &nbsp;
                    <a href="./edit.html?id=${fTasks[index].id}"><i class="fa fa-edit"></i> </a>
                    `;
            li.appendChild(link);
            taskList.appendChild(li);
        }
